#!/usr/bin/env bash
# Publishes wallet.the-coin.cloud (the browser wallet) from this git checkout.
#
# The wallet is static files: keys are derived and transactions signed in the
# browser, and the node only ever receives signed transactions.
#
# First time (copies the files, configures nginx and gets the HTTPS certificate):
#   sudo scripts/deploy-wallet.sh --setup
#
# Updates (after `git pull`):
#   sudo scripts/deploy-wallet.sh
#
# Optional: a different web root as the last argument (default /var/www/wallet.the-coin.cloud).
set -euo pipefail
cd "$(dirname "$0")/.."

DOMAIN=wallet.the-coin.cloud
SETUP=0
...
[ -f wallet/index.html ] || die "run from the repository (wallet/ not found)"
mkdir -p "$WEBROOT" || die "cannot create $WEBROOT (run with sudo)"
[ -w "$WEBROOT" ] || die "cannot write to $WEBROOT (run with sudo)"

info "copying the wallet to $WEBROOT"
if command -v rsync >/dev/null; then
  rsync -a --delete --exclude nginx/ --exclude tests/ --chmod=D755,F644 wallet/ "$WEBROOT/"
else
  rm -rf "${WEBROOT:?}/assets"
  cp -a wallet/index.html wallet/connect.js wallet/assets "$WEBROOT/"
...
EOF
  ...
  cp wallet/nginx/$DOMAIN.conf "$SITE_CONF"
...
echo
info "wallet.the-coin.cloud published (commit $(git -c safe.directory="$PWD" rev-parse --short HEAD 2>/dev/null || echo unknown))"
